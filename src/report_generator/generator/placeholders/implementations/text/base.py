...
"""The type boundary for text placeholders: the base every class in this directory extends, and
the factories for plain and parameterized text."""
...
class AbstractTextPlaceholder(Placeholder, ABC):
    ...
    def resolve_pptx(
        ...
        AbstractTextPlaceholder._resolve_with_adapter(
            AbstractTextPlaceholder._PPTX_ADAPTER, presentation, key, value_cb
        )

    @staticmethod
    def resolve_docx(document: Document, key: str, value_cb: Callable[[], str]) -> None:
        AbstractTextPlaceholder._resolve_with_adapter(
            AbstractTextPlaceholder._DOCX_ADAPTER, document, key, value_cb
        )


def as_multi_parameter_list(
...
def text_placeholder(
    ...
    def decorator(value_func: Callable[[], str]) -> type[Placeholder]:
        class TextPlaceholder(AbstractTextPlaceholder):
...
def parameterized_text_placeholder(
    ...
    def decorator(value_func) -> type[ParameterizedPlaceholder]:
        class ParameterizedTextPlaceholder(
            ParameterizedPlaceholder, AbstractTextPlaceholder
        ):
            __doc__ = value_func.__doc__ if value_func.__doc__ else None
            key = custom_key
