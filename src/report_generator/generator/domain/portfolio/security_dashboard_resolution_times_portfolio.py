...
class SecurityDashboardResolutionTimesPortfolioData:
            ...
            for month_data in system.get("resolutionTimes", []):
                if utils.is_month_in_period(
                    ...
                    for severity_level in ["CRITICAL", "HIGH", "MEDIUM", "LOW"]:
                        ...
                        stats[severity_level]["no_risk"] += severity_data.get(
                            "noRisk", 0
                        )
                        stats[severity_level]["low_risk"] += severity_data.get(
                            "lowRisk", 0
                        )
                        stats[severity_level]["medium_risk"] += severity_data.get(
                            "mediumRisk", 0
                        )
                        stats[severity_level]["high_risk"] += severity_data.get(
                            "highRisk", 0
                        )
    ...
    def _calculate_most_common_times(self, stats):
        ...
        for severity_level in stats:
            ...
            most_days, most_findings = self._find_most_common_resolution_time(
                counts, severity_legend
            )
