...
from report_generator.generator.placeholders.formatting.technologies import (
    _fetch_technologies_yaml,
    _get_technology_cache,
    _load_technologies,
    clear_technology_cache,
    get_cache_info,
    get_fallback_technology_name,
...
MOCK_YAML_DATA = [
    ...
    {
        "context": "abapcds",
        "display_name": "ABAP Core Data Services",
...
class TestFallbackTechnologyName:
    @pytest.mark.parametrize(
        "input_tech,expected", [("js", "JS"), ("python", "Python"), ("cpp", "Cpp")]
    )
...
class TestTechnologyYamlFetch:
    ...
    @patch("requests.get")
    @pytest.mark.parametrize(
        "exception", [RequestException("Network error"), Timeout("Request timeout")]
    )
    ...
    @patch("requests.get")
    @pytest.mark.parametrize(
        "invalid_content", ["invalid: yaml: content: [", yaml.dump({"not": "a list"})]
    )
...
class TestTechnologyLoader:
    @patch(
        "report_generator.generator.placeholders.formatting.technologies._fetch_technologies_yaml"
    )
...
class TestTechnologyLookup:
    ...
    @pytest.mark.parametrize(
        "tech_data,lookup_key,expected_name,expected_category",
        [
    ...
    def test_handles_missing_data_gracefully(
        self, mock_fetch, tech_data, lookup_key, expected_name, expected_category
    ):
