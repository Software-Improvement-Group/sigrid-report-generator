...
class TestRatingDistributionPercentages:
    ...
    def test_rating_distribution_edge_case_exact_thresholds(self, mocker):
        ...
        mock_data = {
            "systems": [
                self._mock_osh_system(
                    "system1", 3.5
                ),  # exactly at upper threshold (above market)
                self._mock_osh_system(
                    "system2", 2.5
                ),  # exactly at lower threshold (market average)
            ]
        }
        mocker.patch.object(
            type(portfolio),
...
class TestWeightedAverageRatings:
    def test_security_weighted_average_rating(self, mocker):
        ...
        mocker.patch(
            "report_generator.generator.domain.portfolio.shared.utils.get_volume",
    ...
    def test_security_weighted_average_with_zero_volume(self, mocker):
        ...
        mock_data = [
            {"systemName": "system1", "rating": 4.0},
            {
                "systemName": "system2",
                "rating": 5.0,
            },  # Should be excluded due to zero volume
        ]
        mocker.patch.object(
            type(portfolio),
            "data",
            ...
            "systems": [
                {
                    ...
                    "sbom": {
                        "metadata": {
                            "properties": [
                                {"name": "sigrid:ratings:system", "value": "4.0"}
                            ]
                        }
                    },
                },
                {
                    ...
                    "sbom": {
                        "metadata": {
                            "properties": [
                                {"name": "sigrid:ratings:system", "value": "3.0"}
                            ]
                        }
                    },
                },
